...
# Initialize the Zone Scanner strategy
strategy = ZoneScanner(
    min_score=_lp["min_score"],
    rr_ratio=_lp["rr_ratio"],
    max_base_candles=_lp["max_base_candles"],
)
logger.info("Active strategy: Supply & Demand Zones (ZoneScanner)")
...
def auto_scan_zones():
    """
    Scan for new Supply & Demand Zone setups and create orders.
    Uses all available slots up to MAX_OPEN_POSITIONS.
    """
    pending_orders = db.get_pending_orders()
    open_trades = db.get_open_trades()
    ...
    available_slots = MAX_OPEN_POSITIONS - total_active
    logger.info(f"Scanning for zone setups... Available slots: {available_slots}")
    ...
    for symbol in symbols_to_scan:
        ...
        try:
            ...
            # In paper mode: if price is already at zone, execute immediately
            if PAPER_TRADING_MODE:
                try:
                    current_price = data_fetcher.get_current_price(symbol)
                    if current_price > 0:
                        ...
                        if proximity <= ZONE_PROXIMITY_PCT:
                            trade_id = db.save_trade(
                                ...
                                strategy="Supply & Demand Zones", reason=best.reasoning,
                            )
            ...
            # Standard: create pending order
            order_id = db.save_pending_order(
                symbol=symbol, side=best.side, quantity=quantity,
                entry_price=best.entry, stop_loss=best.stop_loss,
                target=best.target, strategy="Supply & Demand Zones", reason=best.reasoning,
            )
            active_symbols.add(symbol)
            new_orders += 1
            logger.info(f"📋 Zone: #{order_id} | {best.side} {symbol} @ ₹{best.entry:.2f} (Score: {best.score})")
