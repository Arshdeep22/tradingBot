...
def save_recommendations_log(today: str, all_setups: list, ai_output: dict,
                              display_items: list, placed: list, regime=None):
...
def main():
    ...
    # Load yesterday's feedback (generated by report_generator.py)
    yesterday_feedback = None
    feedback_path = f"reports/{yesterday}_feedback.json"
    if os.path.exists(feedback_path):
        try:
            with open(feedback_path) as f:
                yesterday_feedback = json.load(f)
            logger.info(f"Loaded yesterday's feedback from {feedback_path}")
        except Exception as e:
            logger.warning(f"Could not load yesterday's feedback: {e}")

    # Detect current market regime
    regime = None
    try:
        regime = detect_regime()
        logger.info(f"Market regime: {regime.regime} | ADX={regime.adx} | VIX={regime.vix}")
    except Exception as e:
        logger.warning(f"Regime detection failed: {e}")

    # Load weekly insights (last weekly review if available)
    ...
    if os.path.exists(weekly_dir):
        try:
            ...
            if weekly_files:
                with open(os.path.join(weekly_dir, weekly_files[0])) as f:
                    latest_weekly = json.load(f)
                weekly_insights = latest_weekly.get("executive_summary", "")
                logger.info(f"Loaded weekly insights from {weekly_files[0]}")
        except Exception as e:
    ...
    # Step 3: Get AI recommendations (with regime + feedback context)
    logger.info(f"\n--- STEP 3: Asking AI to rank {len(candidates)} candidates ---")
    ai_output = None
    try:
    ...
    logger.info(f"\n--- STEP 4: Placing top {TOP_N_ORDERS} as pending orders ---")
    ...
    # Step 5: Save recommendations log (includes regime context)
