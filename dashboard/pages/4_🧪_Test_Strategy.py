"""
Test Strategy Page - Backtest Zone Scanner strategy
"""
import sys
import os
...
from strategies import ZoneScanner
...
def create_backtest_chart(report):
    ...
    # Draw entry/SL/target lines for each setup
...
# ==================== PAGE ====================
st.title("🧪 Test Strategy — Supply & Demand Zones")
st.caption("Backtest Zone Scanner — Build setups on past data, forward-test on subsequent data")
...
with st.sidebar:
    ...
    # ---- Zone Scanner parameters ----
    st.subheader("🎛️ Zone Parameters")
...
def _build_strategy(params, use_ai=False, ai_lp=None):
    """Instantiate Zone Scanner from params."""
...
if run_bt:
    ...
    _ai_lp = _lp if use_ai_params else None
    strategy = _build_strategy(strategy_params, use_ai_params, _ai_lp)
...
if reports:
    ...
    st.markdown("## 📊 Backtest Summary — *Supply & Demand Zones*")
    ...
    for ri, report in enumerate(reports):
        ...
        lbl = f"{icon} {report.symbol} | Zones: {report.total_zones_found} | WR: {round(report.win_rate)}%"
    ...
    if run_ai:
        try:
            from core.llm_advisor import create_llm_from_secrets, StrategyAdvisor
            ...
            for iteration in range(1, max_iterations + 1):
                ...
                ai_progress.progress(
                    iteration / max_iterations,
                    text=f"Iteration {iteration}: score={new_params['min_score']} R:R={new_params['rr_ratio']}..."
                )
    ...
    if 'ai_iterations' in st.session_state and st.session_state['ai_iterations']:
        ...
        for it in iterations[1:]:
            ...
            with st.expander(label, expanded=(it['iteration'] == len(iterations) - 1)):
                ...
                st.markdown(f"**Confidence:** {it.get('confidence', 0)}/10")
                p = it["params"]
    ...
    st.markdown("""
    ### How it works:
    1. **Build Period** — Zone Scanner detects supply/demand zones on historical 15m candles
    2. **Test Period** — forward price action checks if zones get triggered
    3. **Results** — see target hits, stop losses, and win rate
    4. **AI Refinement** — LLM optimizes zone parameters automatically
    """)
