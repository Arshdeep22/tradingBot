"""
Test Strategy Page - Backtest Supply & Demand Zones
"""
import sys
import os
...
def create_backtest_chart(report):
    ...
    for zone in report.zones:
        ...
        fig.add_shape(type="rect", x0=zone.formed_at_index, x1=total_len - 1,
                      y0=zone.zone_bottom, y1=zone.zone_top, fillcolor=fc, line=dict(color=lc, width=1))
        ...
        fig.add_hline(y=zone.entry, line_dash="dot", line_color="#2196F3", line_width=1)
        fig.add_hline(y=zone.stop_loss, line_dash="dot", line_color="#F44336", line_width=1)
        fig.add_hline(y=zone.target, line_dash="dot", line_color="#4CAF50", line_width=1)
...
st.caption("Backtest Supply & Demand Zones — Build zones on past data, forward-test on subsequent data")
...
if run_bt:
    ...
    backtester = Backtester(min_score=min_score, rr_ratio=rr_ratio)
    if use_ai_params and _has_ai:
        backtester.zone_scanner.max_base_candles = _lp["max_base_candles"]
    all_reports = []
...
if reports:
    ...
    m1.metric("Zones Found", total_zones)
    ...
    for ri, report in enumerate(reports):
        ...
        lbl = icon + " " + report.symbol + " | Zones: " + str(report.total_zones_found) + " | WR: " + str(round(report.win_rate)) + "%"
        with st.expander(lbl, expanded=(ri == 0)):
            ...
            sc1.metric("Zones", report.total_zones_found)
            ...
            if report.trade_results:
                ...
                for tr in report.trade_results:
                    rows.append({
                        "Type": tr.zone.zone_type,
                        "Score": tr.zone.score,
                        "Entry": round(tr.zone.entry, 2),
                        "SL": round(tr.zone.stop_loss, 2),
                        "Target": round(tr.zone.target, 2),
                ...
                st.info("No zones found in building period.")
    ...
    if run_ai:
        try:
            ...
            # Always fetch max data so AI can explore wider date ranges
            full_data_cache = {}
            ...
            # Start from best known params if they beat current settings
            start_params = {
                "min_score": min_score,
                "rr_ratio": rr_ratio,
                "max_base_candles": 5,
                ...
                for r in rpts:
                    for tr in r.trade_results:
                        res["trade_details"].append({
                            "type": tr.zone.zone_type, "score": tr.zone.score,
                            "outcome": tr.outcome, "pnl": tr.pnl, "rr": tr.rr_achieved,
                        })
                return res, tz, tt, tw, tl, wr, pnl

            def _run_with_params(params):
                bt = Backtester(min_score=params["min_score"], rr_ratio=params["rr_ratio"])
                if "max_base_candles" in params:
                    bt.zone_scanner.max_base_candles = params["max_base_candles"]
    ...
    if 'ai_iterations' in st.session_state and st.session_state['ai_iterations']:
        ...
        fig_iter.add_trace(go.Scatter(
            x=iters_x, y=wr_y, mode='lines+markers', name='Win Rate %',
            line=dict(color='#4CAF50', width=3), marker=dict(size=10),
            yaxis='y'
        ))
        fig_iter.add_trace(go.Scatter(
            x=iters_x, y=pnl_y, mode='lines+markers', name='P&L (₹)',
        ...
        # Show AI analysis for each iteration
    ...
    st.markdown("""
    ### How it works:
    1. **Build Period**: Zones are detected on historical 15m candles
    2. **Test Period**: Forward price action checks if zones get triggered
    3. **Results**: See target hits, stop losses, and win rate
    4. **AI Refinement**: LLM analyzes results and suggests better parameters

    ### Tips:
    - Use 7-10 days for building, 2-3 days for testing
    - Lower Min Score = more zones (lower quality)
    - Higher R:R = bigger targets, fewer hits
    - Click "Run AI Refinement" after backtest to auto-optimize
    """)
